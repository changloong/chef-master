...
/**
 * @App\Cloud\Annotation\Config("redis")
 */
class Redis extends \App\Cloud\Configuration\Recipe {

    protected function configRootNode(\Symfony\Component\Config\Definition\Builder\NodeDefinition $root) {
        self::configRedisRootNode($root, $this, array(
    ...
    public static function configInstallNode(\Symfony\Component\Config\Definition\Builder\NodeDefinition $node, \App\Cloud\Configuration\Recipe $self, array $defaults = array(), $is_default = false){
        ...
        $node
            ->appendIf(
                $self
                    ->createNode('src_url')
                    ->defaultValue('http://192.168.10.20/chef/redis-2.8.14.tar.gz')
                , $is_default
            )
            ->appendIf(
                $self->createNode('src_package')->defaultValue('redis-2.8.14')
                , $is_default
            )
            ->append(
    ...
    public static function configRedisRootNode(\Symfony\Component\Config\Definition\Builder\NodeDefinition $root, \App\Cloud\Configuration\Recipe $self, array $defaults = array(), $is_default = false ){
